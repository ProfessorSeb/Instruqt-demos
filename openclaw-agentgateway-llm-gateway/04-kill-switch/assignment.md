...
teaser: Cut off LLM access instantly without touching your application — and restore it just as fast.
notes:
- type: text
  contents: |-
    # ☠️ The Kill Switch
...
```bash
kubectl delete aiRoute openai-route -n agentgateway-system
```
...
alias llm-kill="kubectl delete aiRoute openai-route -n agentgateway-system --ignore-not-found"
alias llm-restore="kubectl apply -f /root/openai-route.yaml"
alias llm-status="kubectl get aiRoute,gateway -n agentgateway-system"
EOF
